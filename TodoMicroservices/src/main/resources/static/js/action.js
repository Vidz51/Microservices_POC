...
function showToDos() {
    ...
    $.ajax({                                                                        /* Ajax call to AccountMicroservice */
        url: 'http://localhost:8181/showToDos',
        type: "POST",
    ...
    $.ajax({                                                                        /* Ajax call to AccountMicroservice */
        url: 'http://localhost:8970/getStats',
        type: "POST",
        data: {
          jwt: getCookie("jwt"),
          email: $("#email").val()
 ...
 function deleteToDo(id) {
    ...
    $.ajax({
       url: 'http://localhost:8181/deleteToDo/' + id,
       type: "POST",
       success: function (data, status, xhr) {
          console.log(data);
...
$("#submit").click(function(e){
	...
	$.ajax({
		url: 'http://localhost:8181/login',     /* Ajax call to TodoMicroservice for login */
		type: 'POST',
		data: {
			email: $("#email").val(),
...
$("#newToDo").click(function(e) {
    ...
    if( $("#newToDoPriority").val() == null ){
        ...
        $.ajax({                                                        /* Ajax call to AccountMicroservice for login */
           url: 'http://localhost:8181/newToDo',
           type: "POST",
           data: {
